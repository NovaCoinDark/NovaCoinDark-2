...
uint256 hashGenesisBlock("0x0000031b4636eb4e82a3df69121d388f39ab4b58405f4bbbbca656f66fc8f5be"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // novacoindark: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "novacoindark Signed Message:\n";
...
int64 nMinimumInputValue = DUST_HARD_NVCDIT;
...
{
    // novacoindark: IsDust() detection disabled, allows any valid dust to be relayed.
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    // novacoindark
    // To limit dust spam, add nBaseFee for each output less than DUST_SOFT_NVCDIT
    BOOST_FOREACH(const CTxOut& txout, vout)
        if (txout.nValue < DUST_SOFT_NVCDIT)
            nMinFee += nBaseFee;
...
if(nHeight == 1) { nSubsidy = 5500000 * COIN; }                      //PREMINE
    else if(nHeight <= 1001 && nHeight > 1) { nSubsidy = 5000 * COIN; }
else if(nHeight <= 5001 && nHeight > 1001) { nSubsidy = 500 * COIN; }
else if(nHeight > 5001) { nSubsidy = 50 * COIN; }
...
static const int64 nTargetTimespan = 2 * 60 * 60; // novacoindark: 
static const int64 nTargetSpacing = 120; // novacoindark: 
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // novacoindark: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, novacoindark - DarkGravity, written by Evan Duffield - evan@novacoindark.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave2(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, novacoindark - DarkGravity v2, written by Evan Duffield - evan@novacoindark.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // novacoindark: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // novacoindark: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000001c6b46f380e062a7ecc340b0442bc282e0340d7ba787fc7c0693b19bda2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "2014-11-02 -- NovaCoinDark the astronomy term for several types of cosmic explosions...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1415044634;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 399547;

        if (fTestNet)
        {
            block.nTime    = 1414872662;
            block.nNonce   = 5534784;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x865bbebe180410069a4d0ada5c970c3edc2fd9d2923a9f86b23496dccecadace"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa3, 0xd5, 0xc2, 0xf9 }; // novacoindark: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("novacoindarkMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("novacoindarkMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static novacoindarkMiner(CWallet *pwallet)
{
    printf("novacoindarkMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("novacoindark-miner");
    ...
    try { loop {
        ...
        printf("Running novacoindarkMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("novacoindarkMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&novacoindarkMiner, pwallet));
}
